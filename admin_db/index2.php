...
<body>
  ...
  <div class="content">
    ...
    <div id="historical-production" class="block">
      <div class="block-header">Historical Demand Data</div>
      <div class="block-content">Yields, acreage, and costs over time.</div>

      <!-- Date Filter Form -->
      <form method="GET" id="historicalDateFilterForm" class="date-filter-form">
        <label for="historicalStartDate">Start Date:</label>
        <input type="date" id="historicalStartDate" name="startDate">
        <label for="historicalEndDate">End Date:</label>
        <input type="date" id="historicalEndDate" name="endDate">
        <button type="button" id="historicalFilterBtn" class="btn">Filter</button>
      </form>

      <!-- Table to display filtered data -->
      <table id="historicalDataTable" class="table table-bordered">
        <thead>
          <tr>
            <th>Product ID</th>
            <th>Product Name</th>
  ...
  <script>
    document.getElementById('historicalFilterBtn').addEventListener('click', function() {
      ...
      const xhr = new XMLHttpRequest();
      xhr.open('GET', `fetch_historical_data.php?startDate=${startDate}&endDate=${endDate}`, true);
      xhr.onreadystatechange = function() {
        if (xhr.readyState === 4 && xhr.status === 200) {
          const response = JSON.parse(xhr.responseText);
          updateHistoricalTable(response.tableData); // Update table
          updateHistoricalPieChart(response.chartData); // Update pie chart
        }
      };
      xhr.send();
    });
    ...
    let historicalPieChart;

    function updateHistoricalPieChart(chartData) {
      ...
      historicalPieChart = new Chart(ctx, {
        type: 'pie',
        data: {
